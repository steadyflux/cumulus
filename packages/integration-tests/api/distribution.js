...
/**
 * Invoke distribution API lambda directly to get a response payload.
 *
...
async function invokeApiDistributionLambda(path, accessToken = '') {
  deprecate('@cumulus/integration-tests/api/distribution.invokeApiDistributionLambda', '1.18.0');
...
async function getDistributionApiRedirect(filepath, accessToken) {
  deprecate('@cumulus/integration-tests/api/distribution.getDistributionApiRedirect', '1.18.0');
...
async function getDistributionApiFileStream(filepath, accessToken) {
  deprecate('@cumulus/integration-tests/api/distribution.getDistributionApiFileStream', '1.18.0');
...
/**
 * Invoke TEA lambda directly to get the response payload.
 *
 * This is used in integration testing so that we use the lambda's IAM
 * role/permissions when accessing resources.
 *
 ...
 *   Stream to the file protected by the distribution
 */
async function getTEADistributionApiFileStream(filepath, accessToken) {
  const s3SignedUrl = await getTEADistributionApiRedirect(filepath, accessToken);
  return got.stream(s3SignedUrl);
}
